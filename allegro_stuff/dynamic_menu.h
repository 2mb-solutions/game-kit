...
public:
	...
	~dynamic_menu(void);
	int add_item_sound(string filename, int pos, int (*func)(dynamic_menu *m, int posi, void*) = NULL, void* arg = NULL);
	int add_item_text(string text, int pos, int (*func)(dynamic_menu *m, int posi, void*) = NULL, void* arg = NULL);
	int get_item_count();
	int get_position();
